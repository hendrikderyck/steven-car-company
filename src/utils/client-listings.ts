...
import type { ApiListing } from './api-listing-to-car';
...
/**
 * Get CORS proxy URL if configured, otherwise return empty string
 */
function getCorsProxy(): string {
  return import.meta.env.PUBLIC_CORS_PROXY || '';
}
...
function wrapWithProxy(url: string): string {
  ...
  if (proxy) {
    ...
    if (proxy.includes('{url}') || proxy.includes('{{url}}')) {
      return proxy.replace(/\{+\{?url\}?\}+/g, encodeURIComponent(url));
    }
    // If proxy already ends with =, just append the encoded URL directly
    if (proxy.endsWith('=')) {
      return proxy + encodeURIComponent(url);
    }
    // If proxy ends with &, append the encoded URL
    if (proxy.endsWith('&')) {
      return proxy + encodeURIComponent(url);
    }
    // Some proxies use path-based routing (e.g., cors-anywhere style)
    if (proxy.endsWith('/') && !proxy.includes('?')) {
      return proxy + encodeURIComponent(url);
    }
    // Query parameter based - check if url= already exists
    if (proxy.includes('url=')) {
      // Proxy already has url= parameter, just append the URL with &
      return proxy + '&' + encodeURIComponent(url);
    }
    // No url= parameter, add it
...
function extractRelevantFields(ldJson: any, url: string): ApiListing | null {
  let product = null;
...
async function fetchListingDetails(url: string): Promise<ApiListing | { url: string; error: string }> {
  try {
    const proxiedUrl = wrapWithProxy(url);
    // Some CORS proxies don't allow custom headers, so only include them if not using proxy
    const headers = getCorsProxy() ? {} : FETCH_HEADERS;
    ...
    // Some proxies return JSON-wrapped content (e.g., {contents: "..."})
...
/**
 * Fetch only listing URLs (without detail data) - much faster for finding a specific listing
 */
export async function fetchAllListingUrlsClient(): Promise<string[]> {
  try {
    const baseUrl = normalizeDealerUrl(DEALER_URL);
    const seen = new Set<string>();

    // Fetch all pages and extract URLs only
    let page = 1;
    while (page <= 50) {
      const pageUrl = new URL(baseUrl);
      pageUrl.searchParams.set('page', String(page));

      const proxiedUrl = wrapWithProxy(pageUrl.toString());
      const headers = getCorsProxy() ? {} : FETCH_HEADERS;
      const res = await fetch(proxiedUrl, {
        headers,
      });
...
/**
 * Fetch all listings from AutoScout24 client-side with pagination support.
 * Returns listings in ApiListing format compatible with existing transformers.
 */
export async function fetchAllListingsClient(): Promise<ApiListing[]> {
  try {
    const baseUrl = normalizeDealerUrl(DEALER_URL);
    const seen = new Set<string>();
    ...
    for (let i = 0; i < listingUrls.length; i += batchSize) {
      ...
      for (const result of results) {
        if ('error' in result) {
          ...
          if (htmlWrapper) {
            result.htmlWrapper = htmlWrapper;
          }
