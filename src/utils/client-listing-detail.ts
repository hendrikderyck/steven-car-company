import type { DetailPageResult, KeySpec } from './listing-detail';
...
/**
 * Fetch an AutoScout24 listing detail page client-side, extract only car content, return title and HTML.
 */
export async function fetchDetailPageHtmlClient(
  listingUrl: string
): Promise<DetailPageResult> {
  const proxiedUrl = wrapWithProxy(listingUrl);
